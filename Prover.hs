...
module Prover (atom, nt, proofToJson, prove, (&&&), (-->), (|||)) where
...
removeDuplicates xs = case xs of
  [] -> []
  x : xs ->
    let newXs = removeDuplicates xs
     in if x `elem` xs then newXs else x : newXs

combineLists l1 l2 = removeDuplicates (l1 ++ l2)

expandNotL (Sequent (assumps, goals)) =
  let notAssumps = filter (\case Not _ -> True; _ -> False) assumps
   in Sequent
...
expandAndL (Sequent (assumps, goals)) =
  let andAssumps = filter (\case And _ -> True; _ -> False) assumps
   in Sequent
        ( combineLists
...
expandAndR (Sequent (assumps, goals)) =
  case find (\case And _ -> True; _ -> False) goals of
    Just (And (p, q)) ->
      let newGoals = listWithout goals [And (p, q)]
       in [ Sequent
              ( assumps,
...
expandOrL (Sequent (assumps, goals)) =
  case find (\case Or _ -> True; _ -> False) assumps of
    Just (Or (p, q)) ->
      let newAssumps = listWithout assumps [Or (p, q)]
       in [ Sequent
...
expandOrR (Sequent (assumps, goals)) =
  let orGoals = filter (\case Or _ -> True; _ -> False) goals
   in Sequent
        ( assumps,
          combineLists
...
expandImpliesL (Sequent (assumps, goals)) =
  case find (\case Implies _ -> True; _ -> False) assumps of
    Just (Implies (p, q)) ->
      let newAssumps = listWithout assumps [Implies (p, q)]
       in [ Sequent
              ( newAssumps,
...
proveSequent (Sequent (assumps, goals)) =
  let proveAux (Sequent (assumps, goals)) =
        let sequent = Sequent (assumps, goals)
         in if allAtoms assumps && allAtoms goals
              then
                ( if haveIntersection assumps goals
                    then Basic sequent
                    else Invalid sequent
                )
              else
                if any (\case Not _ -> True; _ -> False) assumps
                  then Step (NotL, sequent, [proveAux (expandNotL sequent)])
                  else
                    if any (\case Not _ -> True; _ -> False) goals
                      then Step (NotR, sequent, [proveAux (expandNotR sequent)])
                      else
                        if any (\case And _ -> True; _ -> False) assumps
                          then Step (AndL, sequent, [proveAux (expandAndL sequent)])
                          else
                            if any (\case And _ -> True; _ -> False) goals
                              then Step (AndR, sequent, map proveAux (expandAndR sequent))
                              else
                                if any (\case Or _ -> True; _ -> False) assumps
                                  then Step (OrL, sequent, map proveAux (expandOrL sequent))
                                  else
                                    if any (\case Or _ -> True; _ -> False) goals
                                      then Step (OrR, sequent, [proveAux (expandOrR sequent)])
                                      else
                                        if any (\case Implies _ -> True; _ -> False) assumps
                                          then Step (ImpliesL, sequent, map proveAux (expandImpliesL sequent))
                                          else
                                            if any (\case Implies _ -> True; _ -> False) goals
                                              then Step (ImpliesR, sequent, [proveAux (expandImpliesR sequent)])
                                              else Invalid sequent -- This shouldn't ever be reached
...
sequentToJson (Sequent (assumps, goals)) =
  let assumpsJson = "[" ++ intercalate "," (map propositionToJson assumps) ++ "]"
   in let goalsJson = "[" ++ intercalate "," (map propositionToJson goals) ++ "]"
       in "{\"assumps\": " ++ assumpsJson ++ ", \"goals\": " ++ goalsJson ++ "}"
...
proofToJson proof = case proof of
  ...
  Step (rule, sequent, proofList) ->
    let ruleString = ruleToString rule; sequentJson = sequentToJson sequent; proofJson = "[" ++ intercalate "," (map proofToJson proofList) ++ "]"
     in "{\"type\": \"" ++ ruleString ++ "\", \"sequent\": " ++ sequentJson ++ ", \"proof\": " ++ proofJson ++ "}"